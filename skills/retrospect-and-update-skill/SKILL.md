...
7. Check the skill version in `SKILL.md` frontmatter. If `version` exists, bump it. If it does not, add it before finishing the update.
...
- **Skill updates must carry version intent.** After changing any skill, inspect `SKILL.md` frontmatter. Bump `version` when present; add it when absent, using the next sensible patch version for that skill line.
